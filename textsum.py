...
def get_input_files(corpus_raw):
    book_filenames = sorted(glob.glob("C:/Users/Hp/OneDrive/Desktop/nwin.txt"))
...
def get_summary_files(corpus_raw):
    book_filenames = sorted(glob.glob("C:/Users/Hp/OneDrive/Desktop/nwou.txt"))
    for book_filename in book_filenames:
        # print("Reading '{0}'...".format(book_filename))
        with open(book_filename, "r") as book_file:
            corpus_raw += book_file.read()
...
def normalize_shit(all_sentences, max_avg_tfsif, max_nNouns, max_sentLen):
    for sentence in all_sentences:
        sentence.avg_tfisf /= (max_avg_tfsif+1)
        sentence.n_nouns /= (max_nNouns+1)
        sentence.slen /= (max_sentLen+1)
...
for i in flabel_list:
     if(sum(flabel_list)>0):
       prob_flabel.append(float(i) / sum(flabel_list))
     else:
       prob_flabel.append(float(i))
...
val = math.ceil(float(ratio) * len(ip_array))
# print val
